...
import AddToQueueStyle from '../styles/AddToQueueStyle';
  ...
  render() {
    ...
    return (
      <ul className="add-to-queue__search-results">
        <style jsx>{AddToQueueStyle}</style>
        {results.map((r, index) => {
...
class AddToQueue extends Component {
  ...
  render() {
    const placeholder = 'Add a song';
    const results = this.props.search.results;

    return (
      <div className="add-to-queue" onBlur={this.handleBlur}>
        <style jsx>{AddToQueueStyle}</style>
        <input className="add-to-queue__input" placeholder={placeholder} value={this.state.text} onChange={this.handleChange} onKeyDown={this.handleKeyDown} onFocus={this.handleFocus}/>
