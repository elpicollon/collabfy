...
export const mutePlayback = () => ({ 
  type: types.MUTE_PLAYBACK 
});

export const unmutePlayback = () => ({ 
  type: types.UNMUTE_PLAYBACK 
});
...
export const fetchPlayingContext = () => dispatch => fetch(`${Config.HOST}/api/now-playing`).then(res => res.json()).then(res => dispatch(fetchPlayingContextSuccess(res)));
